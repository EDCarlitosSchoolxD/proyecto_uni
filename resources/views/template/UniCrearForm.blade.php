...
<div>
    ...
    <select  class="form-control" name="tipo_id" id="tipo">
        @foreach ($datos['tipos'] as $tipo)
            <option {{isset($datos['universidad']['universidad']->tipo_id)?'selected':''}} value="{{$tipo->id}}">{{$tipo->tipo}}</option>
        @endforeach
    </select>
</div>
<div>
    ...
    <select  class="form-control" name="lugar_id" id="lugar">
        @foreach ($datos['lugares'] as $lugar)
            <option {{isset($datos['universidad']['universidad']->lugar_id)?'selected':''}} value="{{$lugar->id}}">{{$lugar->nombre}}</option>
        @endforeach
    </select>
</div>
<div>
    ...
    <select class="form-control" name="estado_id" id="estados">
        ...
        @foreach ($datos['estados'] as $estado)
            <option {{isset($datos['universidad']['universidad']->estado_id)?'selected':''}} value="{{$estado->id}}">{{$estado->nombre}}</option>
         @endforeach
        </select>
    </div>
<div>
